...
class FasterRCNN_prepare():
    ...
    def forward(self, is_training=True, is_da=False):
        ...
        if is_training:
            if is_da:
                self.s_imdb_train, s_roidb_train, s_ratio_list_train, s_ratio_index_train = combined_roidb_da(s_imdb_name)
            else:
            ...
            if is_da:
                s_dataset_train = roibatchLoader_da(s_roidb_train, s_ratio_list_train, s_ratio_index_train,
                                                 self.batch_size_train, \
                                                 self.s_imdb_train.num_classes, training=True, is_target=False)
            else:
                s_dataset_train = roibatchLoader_da(s_roidb_train, s_ratio_list_train, s_ratio_index_train,
                                                 self.batch_size_train, \
        ...
        if is_da:
            self.s_imdb_test, s_roidb_test, s_ratio_list_test, s_ratio_index_test = combined_roidb_da(s_imdbval_name,
                                                                                                   False)
        else:
        ...
        if is_da:
            dataset_test = roibatchLoader_da(s_roidb_test, s_ratio_list_test, s_ratio_index_test, self.batch_size_test, \
                                          self.s_imdb_test.num_classes, training=False, normalize=False, is_target=False)
        else:
...
def eval_frcnn(frcnn_extra, device, fasterRCNN, is_debug=False):
